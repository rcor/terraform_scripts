...
public_subnet=`echo $public_subnet_from_output | tr -d [],\"`
echo "Add tag to $vpc"
...
do
  ...
  eval "aws ec2 create-tags --resources $i --tags Key=kubernetes.io/cluster/$name,Value=shared";
  eval "aws ec2 create-tags --resources $i --tags Key=kubernetes.io/role/internal-elb,Value=1";
done

for b in  $public_subnet
do
  echo "Add tag to $b:elb"
  eval "aws ec2 create-tags --resources $b --tags Key=kubernetes.io/cluster/$name,Value=shared";
  eval "aws ec2 create-tags --resources $b --tags Key=kubernetes.io/role/elb,Value=1";
done
